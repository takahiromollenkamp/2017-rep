...
public class Play2 extends Core implements KeyListener{
	...
	public synchronized void draw(Graphics2D g){
		...
		mess="Score: "+score+" Life: "+life;
		g.drawString(mess, 2, 15);
		} else{
			g.clearRect(0, 0,2000,2000 );
			 mess="Your score was "+finalScore+". You died on wave "+wave;
	...
	public void update(long timePassed){
		...
		if(spacePressed&&shoot()){
			beam[shotCount%10].activate(ship.getX()+16,ship.getY()-30);
			shotCount++;
		...
		for(int k=0;k<10;k++){
			if(empire[k].isAlive()&&empire[k].canShoot()){
				...
				empire[k].getEvilShot(empire[k].getShot()%10).activate(empire[k].getX()+15, empire[k].getY()+30);
				empire[k].incShot();
	...
	public void spawnAliens(){
		empire=new Alien[10];
		for(int i=0;i<10;i++){
			empire[i]=new Alien(20+60*i, 20,.1f+.01f*wave);
		}
		ship =new Ship();
	...
	public void init(){
		...
		empire=new Alien[10];
		for(int i=0;i<10;i++){
			empire[i]=new Alien(20+60*i, 20,.04f);
			empire[i].delayTime();
		}
		shotCount=0;
		leftPressed=false;
